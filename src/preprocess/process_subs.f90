...
!-------------------------------------------------------------------------
  ...
  call MPI_Bcast(cmt_lat,1,MPI_DOUBLE,0,comm,ierr)
  call MPI_Bcast(cmt_lon,1,MPI_DOUBLE,0,comm,ierr)
...
subroutine cut_seis(obsd, npts1, dt1, b1, synt, npts2, dt2, b2, &
    ...
    integer :: cuterr, nerr, nfillb=0, nfille=0
    ...
    beg_max=max(b1,b2)+1
    print *,"b1, b2, dt1, dt2, beg_max:", b1, b2, dt1, dt2, beg_max
    ...
    enddo
    npoints_start = jj 
    !npoints_start = (beg_max-b1)/dt1
    npoints_end = j
    print *, "another way:", (beg_max-b1)/dt1
    !print *, "test pur:", time_observed(1), time_observed(2), time_observed(npoints_start)
    print *, "npoints_start, npoints_end:", npoints_start, npoints_end
    print *, "nfillb, nfille:", nfillb, nfille
    ...
    npoints_start = jj
    npoints_end = j
    print *, "synt, npts_start, npts_end:", npoints_start, npoints_end
...
!! \param x The time-series
!! \param n The number of points in the time-series
subroutine detrend(data, npts, yint, slope, b, dt)

  integer, intent(in) :: npts
  real(kind=4), dimension(*) :: data
  real(kind=4) :: yint, slope, b, dt
  real(kind=4) :: siga, sigb, sig, cc

  real(kind=8) :: b_double, dt_double
...
subroutine rotate_traces(my_asdf, my_asdf_rotate)
    ...
    do i=1, num_stations
      ...
      call locate_record(sta_list(i), nw_list(i), "E", my_asdf%receiver_name_array, &
        my_asdf%network_array, my_asdf%component_array, nrecords, loc_e)
      call locate_record(sta_list(i), nw_list(i), "N", my_asdf%receiver_name_array, &
        my_asdf%network_array, my_asdf%component_array, nrecords, loc_n)
      print *, "loc_e, n, z:", loc_e, loc_n, loc_z
      
      !if found, set the npionts; otherwise, set it to zero
      if(loc_e>0)then
        nn_e=my_asdf%npoints(loc_e)
      else
        nn_e=0
      endif
      if(loc_n>0)then
        nn_n=my_asdf%npoints(loc_n)
      else
        nn_n=0
      endif
      if(loc_z>0)then
      ...
      elseif(loc_n.gt.0)then
        receiver_lat=my_asdf%receiver_lat(loc_n)
        receiver_lo=my_asdf%receiver_lo(loc_n)
        receiver_dpt=my_asdf%receiver_dpt(loc_n)

        azm=my_asdf%ev_to_sta_AZ(loc_n)
        bzm=my_asdf%sta_to_ev_AZ(loc_n)
        tt=my_asdf%begin_value(loc_n)
        dtt=my_asdf%sample_rate(loc_n)
      elseif(loc_e.gt.0)then
        receiver_lat=my_asdf%receiver_lat(loc_e)
        receiver_lo=my_asdf%receiver_lo(loc_e)
        receiver_dpt=my_asdf%receiver_dpt(loc_e)

        azm=my_asdf%ev_to_sta_AZ(loc_e)
        bzm=my_asdf%sta_to_ev_AZ(loc_e)
        tt=my_asdf%begin_value(loc_e)
        dtt=my_asdf%sample_rate(loc_e)
      endif

      !============================================
      !deal with North and East component first
      print *, "Rotate East and North"
      if((loc_e.eq.0).and.(loc_n.eq.0))then
      ...
      elseif((loc_e.eq.0).or.(loc_n.eq.0))then
        ...
        allocate(my_asdf_rotate%records(3*i-2)%record(nn))
        allocate(my_asdf_rotate%records(3*i-1)%record(nn))
        edata(1:nn)=my_asdf%records(loc_e)%record(1:nn)
        ndata(1:nn)=my_asdf%records(loc_n)%record(1:nn)
        !bzm=326.2812
        rotaz=bzm+180
        ...
        !rdata(1:nn) = costh * ndata(1:nn) + sinth * edata(1:nn)
        !tdata(1:nn) = -sinth * ndata(1:nn) + costh * edata(1:nn)
        call rotate(ndata, edata, nn, rotaz, .true., .true., rdata, tdata)
        my_asdf_rotate%records(3*i-2)%record(1:nn)= rdata(1:nn)
        my_asdf_rotate%records(3*i-1)%record(1:nn)= tdata(1:nn)
        deallocate(rdata)
        deallocate(tdata)
        deallocate(edata)
        deallocate(ndata)
...
subroutine store_asdf_metadata(asdf_old, asdf_new)
  ...
  do i=1, asdf_old%nrecords 
    !event info from cmt file
    asdf_new%event_lat(i)=cmt_lat
    asdf_new%event_lo(i)=cmt_lon
    asdf_new%event_dpt(i)=cmt_depth
    ...
    asdf_new%receiver_id_array(i) = asdf_old%receiver_id_array(i)
